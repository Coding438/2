  ...
  return `
    ...
    <div class="card">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:10px;margin-top:8px">
        ...
        <button class="btn btn-secondary" onclick="pullFromSheets(false)"><i class="fas fa-cloud-download-alt"></i> Load from Sheets</button>
...
/** Pull Products + Orders from Google Sheets into localStorage (works on live domain) */
async function pullFromSheets(silent) {
  const base = typeof getSheetsUrl === 'function' ? getSheetsUrl() : (localStorage.getItem('googleSheetUrl') || '');
  if (!base || base.indexOf('script.google.com') === -1) {
    if (!silent) showToast('Google Sheet URL missing', 'error');
    return { products: 0, orders: 0 };
  }
  const result = { products: 0, orders: 0 };
  async function getType(type) {
    const url = base + (base.indexOf('?') >= 0 ? '&' : '?') + 'action=get&type=' + type + '&_=' + Date.now();
    const res = await fetch(url, { method: 'GET', redirect: 'follow', cache: 'no-store' });
    const text = await res.text();
    let json;
    try { json = JSON.parse(text); } catch (e) { throw new Error(type + ': invalid JSON from Sheets'); }
    if (!json || json.ok === false) throw new Error((json && json.error) || (type + ' fetch failed'));
    return Array.isArray(json.data) ? json.data : [];
  }
  try {
    const [products, orders] = await Promise.all([getType('products'), getType('orders')]);
    if (products.length) {
      // normalize imageUrls strings from sheet
      const normalized = products.map(p => {
        let urls = p.imageUrls;
        if (typeof urls === 'string' && urls.trim()) {
          try { urls = JSON.parse(urls); } catch (e) { urls = urls.split(',').map(s => s.trim()).filter(Boolean); }
        }
        if (!Array.isArray(urls)) urls = urls ? [urls] : [];
        if (!urls.length && p.imageUrl) urls = [p.imageUrl];
        return { ...p, imageUrls: urls, imageUrl: p.imageUrl || urls[0] || '' };
      });
      Storage.set('products', normalized);
      if (typeof publishProductsToStorefront === 'function') publishProductsToStorefront(normalized);
      result.products = normalized.length;
    }
    if (orders.length) {
      // merge with local so we do not lose brand-new local-only rows
      const local = Storage.get('orders') || [];
      const byId = new Map();
      local.forEach(o => byId.set(String(o.id), o));
      orders.forEach(o => {
        const id = String(o.id);
        const existing = byId.get(id);
        byId.set(id, existing ? { ...existing, ...o } : o);
      });
      const merged = Array.from(byId.values()).sort((a, b) => String(b.date || '').localeCompare(String(a.date || '')));
      Storage.set('orders', merged);
      result.orders = merged.length;
    }
    if (!silent) {
      showToast('Loaded from Sheets: ' + result.products + ' products, ' + result.orders + ' orders');
    }
  } catch (err) {
    console.warn('pullFromSheets', err);
    if (!silent) showToast('Could not load from Google Sheets: ' + err.message, 'error');
  }
  return result;
}
...
// Live domain: load products + orders from Google Sheets (not only localStorage)
pullFromSheets(true).then(function (r) {
  const active = document.querySelector('.nav-item.active');
  const page = active ? active.dataset.page : 'dashboard';
  navigate(page || 'dashboard');
}).catch(function () {});

// Refresh orders from Sheets every 15s so new website orders appear quickly
setInterval(function () {
  pullFromSheets(true).then(function (r) {
    const active = document.querySelector('.nav-item.active');
    if (!active) return;
    if (active.dataset.page === 'orders' || active.dataset.page === 'dashboard') {
